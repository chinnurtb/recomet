...
ping() ->
    ...
    riak_core_vnode_master:command(IndexNode, ping, recomet_vnode_master).


logout (Pid,Channel,Uid,Type) ->
    riak_core_vnode_master:command(get_pri_node(Channel,Uid), {logout, Pid,Channel,Uid,Type}, recomet_vnode_master),
    ok.

send (Channel,Uid,Type,Message) ->
    riak_core_vnode_master:command(get_pri_node(Channel,Uid), {send, Channel,Uid,Type,Message}, recomet_vnode_master),
    ok.
...
is_online(Channel,Uid1,Type) when is_list(Uid1)->
    ...
    lists:foldl(fun({Node,Uids},List)->
        OnlineList = riak_core_vnode_master:command(Node, {is_online, Channel,Uids,Type}, recomet_vnode_master),
        lists:merge(OnlineList,List)
    end, [],Nodes);

is_online(Channel,Uid,Type ) when is_integer(Uid) ->
    riak_core_vnode_master:command(get_pri_node(Channel,Uid), {is_online, Channel,Uid,Type}, recomet_vnode_master).

login (Pid,Channel,Uid,Type,Ctime) ->
     IndexNode = get_pri_node(Channel,Uid),
    riak_core_vnode_master:command(IndexNode, {login, Pid,Channel,Uid,Type, Ctime}, recomet_vnode_master),
    ok.
